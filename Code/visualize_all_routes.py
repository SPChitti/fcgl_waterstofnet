...
def create_aggregate_map(
    ...
    Args:
        ...
        line_weight: Line thickness (2-4 recommended for many routes)
    """
    print("="*80)
    ...
    origin_colors = {}
    colors_palette = [
        '#c0392b', '#2471a3', '#27ae60', '#d68910', '#7d3c98',
        '#138d75', '#d35400', '#2c3e50', '#117a65', '#922b21',
        '#1e8449', '#1f618d', '#6c3483', '#b9770e', '#884ea0',
        '#5d6d7e', '#1c2833', '#f39c12'
    ]
    ...
    for idx, route_data in enumerate(all_routes):
        ...
        if color_mode == "slope":
            # Draw with slope-based colors - darker and more opaque
            ...
            for i in range(len(points) - 1):
                ...
                # Darker colors and higher opacity for better visibility
                if is_tunnel:
            ...
            # Single color per origin city - darker and more opaque
    ...
    if color_mode == "slope":
        ...
        legend_items = ''.join([
            f'<p style="margin: 2px 0;"><span style="color: {origin_colors[o]}; font-size: 16px;">━━━</span> {o}</p>'
            for o in sorted(origins)[:10]
        ])
        legend_html = f'''
        <div style="position: fixed; 
                    bottom: 50px; left: 50px; width: 240px; max-height: 400px; overflow-y: auto;
                    background-color: white; border:3px solid #2c3e50; z-index:9999; 
                    font-size:13px; padding: 15px; border-radius: 8px; box-shadow: 0 4px 8px rgba(0,0,0,0.2);">
            <h4 style="margin-top:0; color: #2c3e50;">Belgium Logistics Network</h4>
            <p style="color: #34495e;"><b>{routes_plotted} routes (Alternative {show_alternative})</b></p>
            <p style="color: #7f8c8d; font-size: 11px;">{total_segments:,} total segments</p>
            <hr style="border: 1px solid #ecf0f1; margin: 10px 0;">
            <p style="margin: 8px 0; font-weight: bold; color: #2c3e50;">Origins (top 10):</p>
            {legend_items}
...
def main():
    ...
    parser.add_argument("--color-mode", default="slope", choices=["slope", "origin"],
                       help="Color mode: 'slope' for terrain or 'origin' for source city")
    parser.add_argument("--line-weight", type=int, default=3,
                       help="Line thickness (2-4 recommended for many routes)")
